...
#include "treeUtilities.hpp"#include <omp.h>
#include <cmath>
#include <cstdlib>
#include <cstring>
#include <ctime>
    ...
    #pragma omp parallel
    {
        ...
        for (std::size_t i = 0; i < particles.size(); ++i) {
            for (std::size_t j = i + 1; j < particles.size(); ++j) {
                ...
                if (particles[i].squareDistance(particles[j]) < minimumDistance * minimumDistance * softening) {
...
double computeRatio(const std::array<double, Dimension>& pos, const std::array<double, Dimension>& centerMass,
    ...
    for (size_t i = 0; i < Dimension; ++i) {
        const double difference = pos[i] - centerMass[i];
...
void parallelSimulation(int it, std::vector<Particle<Dimension>>* particles, int dim, double softening, double delta_t, Force<Dimension>& f, int u, size_t numFilesAndThreads){
    ...
    */
        ...
        #pragma omp parallel shared(particles, f, it, delta_t, coordinateFiles, softening, dim, num_threads, num_particles) private(force, id_thread, k, q)
    {
        id_thread = omp_get_thread_num();
    {
        for (int z = 0; z < it; ++z){
               // [COLLISION-SYNC] Detect and apply pair collisions outside the force loop.
               #pragma omp single
               {
