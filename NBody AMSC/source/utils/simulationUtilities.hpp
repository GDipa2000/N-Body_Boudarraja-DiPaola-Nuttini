...
std::vector<Particle<Dimension>> generateRandomParticles(int N, int posBoundary = 100, int minProperty = 1,
    ...
    int maxRetry = 15, counter = 0;//massimo numero di tentativi per generare una particella non sovrapposta
    bool overlapping = false; //flag to check if the particle being generated is overlapping with any other particle
    double r, property, squareDistance = 0.0;//raggio particella, proprietà particella, distanza quadratica tra particelle
    ...
    const std::size_t targetCount = N > 0 ? static_cast<std::size_t>(N) : 0; //trasformo N in size_t per evitare warning di conversione
    while (particles.size() < targetCount) {
        ...
        r = disRadius(gen);//genero dimensioni casuali di dimensione

        for (size_t i = 0; i < Dimension; ++i) pos[i] = disPos(gen); //genero posizioni casuali per ogni dimensione della particella

        for (const Particle<Dimension>& p : particles) {
            squareDistance = 0.0;
            for (size_t i = 0; i < Dimension; ++i)
                squareDistance = squareDistance + ((p.getPos()[i] - pos[i]) * (p.getPos()[i] - pos[i])); //calcolo la distanza quadratica tra la particella che sto generando e la particella p del vettore particles
            const double minimumDistance = p.getRadius() + r; //calcolo la distanza minima tra le due particelle, che è la somma dei loro raggi
            if (squareDistance < minimumDistance * minimumDistance) {
                counter++;
                overlapping = true;
                break;
            }
        ...
        if (!overlapping) {
            counter = 0;
            //genero proprietà casuali della particella
            property = disProperty(gen);
            //genero velocità casuali della particella
            for (size_t i = 0; i < Dimension; ++i) vel[i] = disVel(gen);

            Particle<Dimension> p(particles.size(), property, pos, vel, r, type);

            particles.push_back(p); //aggiungo la particella al vettore di particelle
        } 
    }
